...
- 定义函数可以通过`function`关键字进行定义,如`function 函数名(){}`,也可以通过赋值的方式进行定义,如`var/let 函数名 = function(){};`,也可以不给函数取名(称为匿名函数),例如`function(){}`
...
- 函数表达式可以加小括号去执行,函数定义时先加小括号转换为表达式,再在后面加小括号直接执行,称为立即执行函数(IIFE).立即执行函数可以将代码模块化,避免污染全局环境
