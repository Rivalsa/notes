...
`.replace(旧内容,新内容)`用新内容替换旧内容,旧内容可以传入正则表达式,返回替换后的字符串,不改变原字符串.

*新内容也可以为函数,每次匹配到数据后执行函数,函数的返回值为替换为的内容,函数会传一个实参为匹配到的内容(形参通常用$0表示)*
...
`padStart(数字,字符串)`字符串的方法,如果不够对应位数,则在结尾添加参数中指定的字符串,补满位数

`padEnd(数字,字符串)`字符串的方法,如果不够对应位数,则在开头添加参数中指定的字符串,补满位数
...
`DOMMouseScroll`鼠标滚轮事件（火狐浏览器专用，且不支持0级事件）

`mousewheel`鼠标滚轮事件（标准现代浏览器（如谷歌），0级事件与2级事件均可）
...
RegExp中存储了上一次的子项,可以通过这个对象直接拿到数据.(通常先test然后通过RegExp得到子项)
