...
JavaScript中的数字有最大值和最小值的限制,最大值可以利用`Number.MAX_VALUE`取得,若数字大于此值,则为`Infinity`.最小值可以利用`Number.MIN_VALUE`取得,若数字小于此值,则为0
...
可以使用`对象名.属性`的方式来获取对应的值,也可以使用`对象名[属性]`的方式来获取对应的值,但通过第一种方式时属性名必须满足变量的命名规则
...
- 函数可以加小括号去执行
- 函数在定义时经赋值(不仅用`=`可以赋值,给整体加`()`等方式也算一种赋值)可以直接在后面加小括号立即执行,称为立即执行函数(或自执行函数)
...
- 如果`+`两侧有一侧是字符串,则会将另一侧强制转换为字符串类型(symbol类型除外)后拼接为新字符串
- 如果`+`两侧有一侧是symbol类型,则会报错
- 如果`+`两侧都是数字类型,则按照数字相加运算,返回结果为数字类型
- 如果`+`两侧有一侧是数字,另一侧是布尔值或null或undefined则会强制转换为数字类型再运算
- 其他情况一律全部强制转换为字符串再进行运算
